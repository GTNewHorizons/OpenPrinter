    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int slot) {
        ...
        if (slotObject != null && slotObject.getHasStack()) {
            ...
            if (slot >= outputSlotStart && slot < outputSlotEnd) {
                if (!mergeItemStack(stackInSlot, inventoryStart, hotbarEnd, true)) return null;
            ...
            // Try merge stacks within inventory and hotbar spaces
                ...
                for (Slot ss : specialSlots) {
                    if (!tileEntity.isItemValidForSlot(ss.getSlotIndex(), stackInSlot)) continue;
                    handledSpecialItem = mergeItemStack(stackInSlot, ss.slotNumber, ss.slotNumber + 1, false);
                    if (handledSpecialItem) break;
                }
            ...
            else if (!mergeItemStack(stackInSlot, inventoryStart, hotbarEnd, false)) return null;

...
public class GuiFolderView extends GuiScreen {
    ...
    public void drawScreen(int i, int j, float f) {
        ...
        if (stack.hasTagCompound() && stack.getTagCompound().hasKey("ItemInventory", Constants.NBT.TAG_LIST)) {
            ...
            if (this.currPage <= 0) this.buttonPreviousPage.visible = false;
            else this.buttonPreviousPage.visible = true;
            ...
            for (int l = 0; l <= ItemInventory.getCompoundTagAt(this.currPage).getCompoundTag("tag").func_150296_c()
                ...
                if (ItemInventory.getCompoundTagAt(this.currPage).getCompoundTag("tag").getDouble("version") == 2.0) {
                    ...
                    if (parts.length > 1) {
                        ...
                        if (alignment.equalsIgnoreCase("center")) {
                            mc.fontRenderer.drawString(
                                    parts[0],
                                    width / 2 - mc.fontRenderer.getStringWidth(parts[0]) / 2 + 10,
                                    height / 2 - offset,
                                    color);
                        } else {
                            mc.fontRenderer.drawString(
                    ...
                    drawCenteredString(
                            mc.fontRenderer,
                            stack.stackTagCompound.getString("pageTitle"),
                            width / 2,
                            height / 2 - 110,
                            0x000000);
                    String output = ItemInventory.getCompoundTagAt(this.currPage).getCompoundTag("tag")
                    ...
                    if (alignment.equalsIgnoreCase("center")) {
                        mc.fontRenderer.drawString(
                                output,
                                width / 2 - mc.fontRenderer.getStringWidth(output) / 2,
                                height / 2 - offset,
                                color);
                    } else {
                        mc.fontRenderer
    ...
    protected void actionPerformed(GuiButton button) {
        if (button.id == 1 && this.currPage < this.pageCount) this.currPage = this.currPage + 1;

        if (button.id == 2 && this.currPage - 1 >= 0) this.currPage = this.currPage - 1;
    }
